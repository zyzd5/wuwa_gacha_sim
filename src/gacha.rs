...
///
/// 所以这个值是**实测**出来的，不是猜的：用 `tools/gacha_history.py` 统计一位玩家
/// 「角色精准调谐」（限定角色池）6 个月共 310 抽，得到 38 个 4★，其中 **5 个是武器**：
///
/// - 点估计 `5/38 = 13.2%`，Wilson 95% 置信区间 **[5.8%, 27.3%]**
/// - 样本量偏小，区间仍宽；常量取点估计 **0.13**
///
/// 想更准就多收集记录（用法见 `tools/gacha_history.py`），然后改这一个常量。
/// 大样本测试会断言模拟出的占比收敛到这里，改错了会立刻被测出来。
pub const FOUR_STAR_WEAPON_SHARE: f64 = 0.13;
/// 8 个大珊瑚兑换 1 抽。
